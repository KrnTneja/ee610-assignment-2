...
def inverse_filter(image, kernel, th=None):
    image_padded = fft.pad_zeros(image)
    image_fft = fft.fft2d(np.float32(image_padded))

    kernel_padded = np.float32(fft.pad_zeros(kernel, image_padded.shape[0]))
    kernel_fft = fft.fft2d(kernel_padded)

    filter_fft = 1/kernel_fft
    if th != None: filter_fft[np.absolute(kernel_fft) < th] = 1.0
    output_fft = filter_fft*image_fft
    return np.real(fft.ifft2d(output_fft))[:image.shape[0], :image.shape[1]]
...
def cls_filter(image, kernel, gamma):
    ...
    laplacian_padded = np.float32(fft.pad_zeros(np.array([[0.0, -1.0, 0.0], 
        [-1.0, 4.0, -1.0], [0.0, -1.0, 0.0]]), image_padded.shape[0]))
    laplacian_fft = fft.fft2d(laplacian_padded)
