...
def getTruncatedInverseFilter(image, kernel):
    kernel_padded = np.zeros(image.shape, dtype=np.float32)
    kernel_padded[:kernel.shape[0], :kernel.shape[1]] = kernel
    image_fft = np.fft.fft2(np.float32(image))
    kernel_fft = np.fft.fft2(kernel_padded)
    def inverse_filter(th):
        filter_fft = 1/kernel_fft
        filter_fft[np.absolute(kernel_fft) < th] = 1.0
        output_fft = filter_fft*image_fft
        return np.real(np.fft.ifft2(output_fft))    
    return inverse_filter

def getTruncatedInverseFilter2D(image, kernel):
    channel_function_map = []
    for channel in range(image.shape[2]):
        channel_function_map.append((channel, 
            getTruncatedInverseFilter(image[:, :, channel], kernel)))
    def inverse_filter(th):
        output=np.zeros(image.shape)
        for channel, filter_function in channel_function_map:
            output[:, :, channel] = np.real(filter_function(th))
...
def inverseFilter2D(image, kernel):
    output = np.zeros(image.shape)
    for channel in range(image.shape[2]):
        output[:, :, channel] = np.real(inverse_filter(image[:, :, channel], kernel))
    output[output > 255] = 255
    output[output < 0] = 0
    output = np.uint8(output)
    return output
...
def getApproximateWeinerFilter(image, kernel):
    kernel_padded = np.zeros(image.shape, dtype=np.float32)
    kernel_padded[:kernel.shape[0], :kernel.shape[1]] = kernel
    ...
    def weiner_filter(k):
        ...
        output_fft = filter_fft*image_fft
        return np.real(np.fft.ifft2(output_fft))
    return weiner_filter

def getApproximateWeinerFilter2D(image, kernel):
    channel_function_map = []
    for channel in range(image.shape[2]):
        channel_function_map.append((channel, 
            getApproximateWeinerFilter(image[:, :, channel], kernel)))
    def weiner_filter(k):
        output=np.zeros(image.shape)
        for channel, filter_function in channel_function_map:
            output[:, :, channel] = np.real(filter_function(k))
        output[output > 255] = 255
        output[output < 0] = 0
        output = np.uint8(output)
        return output
...
def getConstrainedLSFilter(image, kernel):
    laplacian_padded = np.zeros(image.shape, dtype=np.float32)
    laplacian_padded[:3, :3] = np.array([[0.0, -1.0, 0.0], [-1.0, 4.0, -1.0], [0.0, -1.0, 0.0]])
    kernel_padded = np.zeros(image.shape, dtype=np.float32)
    kernel_padded[:kernel.shape[0], :kernel.shape[1]] = kernel
...
def getConstrainedLSFilter2D(image, kernel):
    channel_function_map = []
    for channel in range(image.shape[2]):
        channel_function_map.append((channel, getConstrainedLSFilter(image[:, :, channel], kernel)))
    def cls_filter(gamma):
        output=np.zeros(image.shape)
        for channel, filter_function in channel_function_map:
            output[:, :, channel] = filter_function(gamma)
        output[output > 255] = 255
        output[output < 0] = 0
        output = np.uint8(output)
        return output
